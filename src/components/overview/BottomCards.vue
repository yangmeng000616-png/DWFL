  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2">
    ...
    <div class="tech-panel rounded-xl py-2 px-2.5 flex flex-col justify-between shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
      <div class="flex items-center justify-between pb-1.5 border-b border-[#184682]/60">
        ...
        <!-- Metric Switcher -->
        <div class="flex items-center bg-[#071d44]/90 p-0.5 rounded-lg border border-[#1d4d8c]/70 text-xs">
          <button
            v-for="m in metricChannels"
            :key="m.id"
            @click="activeMetric = m.id"
            class="px-2 py-0.5 rounded transition-all font-medium text-[10px] cursor-pointer"
            :class="activeMetric === m.id ? 'bg-gradient-to-r from-[#217bf8] to-[#00a6ff] text-white font-semibold shadow-[0_0_8px_rgba(33,123,248,0.6)]' : 'text-slate-400 hover:text-white'"
          >
            {{ m.label }}
          </button>
        </div>
      ...
      <div class="relative mt-1 flex-1 flex flex-col min-h-[115px]">
        <div class="flex items-center justify-between text-[11px] text-slate-300 mb-0.5 flex-shrink-0">
          <div class="flex items-center gap-1.5">
            <span class="w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_5px_#06b6d4]"></span>
        ...
        <div class="flex-1 w-full min-h-[95px] relative">
          <svg class="w-full h-full block" viewBox="0 0 320 110" preserveAspectRatio="none">
            <defs>
              <linearGradient id="trendCyanGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="#00f0ff" stop-opacity="0.5" />
                <stop offset="60%" stop-color="#00a6ff" stop-opacity="0.18" />
                <stop offset="100%" stop-color="#00a6ff" stop-opacity="0.0" />
              </linearGradient>
            ...
            <line x1="26" y1="10" x2="310" y2="10" stroke="#173e73" stroke-dasharray="2 3" />
            <line x1="26" y1="50" x2="310" y2="50" stroke="#173e73" stroke-dasharray="2 3" />
            <line x1="26" y1="90" x2="310" y2="90" stroke="#225494" stroke-width="1" />

            <!-- Y-Axis ticks -->
            <text x="22" y="13" fill="#94a3b8" font-size="8.5" text-anchor="end" font-family="'Chakra Petch', sans-serif">
              {{ currentMetricConfig.yMax }}
            </text>
            <text x="22" y="53" fill="#94a3b8" font-size="8.5" text-anchor="end" font-family="'Chakra Petch', sans-serif">
              {{ currentMetricConfig.yMid }}
            </text>
            <text x="22" y="93" fill="#94a3b8" font-size="8.5" text-anchor="end" font-family="'Chakra Petch', sans-serif">
            ...
            <path
              :d="currentMetricConfig.strokePath"
              fill="none"
              stroke="url(#trendLineGrad)"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
              class="drop-shadow-[0_0_6px_rgba(0,240,255,0.7)]"
            />

            <!-- Peak Annotation Indicator (if exists) -->
            <g v-if="currentMetricConfig.peakPoint">
              <circle :cx="currentMetricConfig.peakPoint.x" :cy="currentMetricConfig.peakPoint.y" r="2.5" fill="#f59e0b" />
              <circle :cx="currentMetricConfig.peakPoint.x" :cy="currentMetricConfig.peakPoint.y" r="5.5" fill="none" stroke="#f59e0b" stroke-width="1" opacity="0.6" />
              <text
                :x="currentMetricConfig.peakPoint.x"
                :y="currentMetricConfig.peakPoint.y - 6"
                fill="#fcd34d"
                font-size="8.5"
                ...
                font-weight="bold"
              >
                {{ currentMetricConfig.peakPoint.label }}
              </text>
            </g>

            <!-- Current/Latest Value Radar Pulse Dot -->
            <g v-if="currentMetricConfig.lastPoint">
              <circle :cx="currentMetricConfig.lastPoint.x" :cy="currentMetricConfig.lastPoint.y" r="2.5" fill="#00f0ff" />
              <circle :cx="currentMetricConfig.lastPoint.x" :cy="currentMetricConfig.lastPoint.y" r="5.5" fill="none" stroke="#00f0ff" stroke-width="1.2" class="animate-ping" opacity="0.75" />
            </g>

            <!-- X-Axis time labels (Cleanly placed below baseline 90) -->
            <text x="35" y="104" fill="#64748b" font-size="8.5" text-anchor="middle" font-family="'Chakra Petch', sans-serif">00:00</text>
            <text x="102" y="104" fill="#64748b" font-size="8.5" text-anchor="middle" font-family="'Chakra Petch', sans-serif">06:00</text>
            <text x="170" y="104" fill="#64748b" font-size="8.5" text-anchor="middle" font-family="'Chakra Petch', sans-serif">12:00</text>
            <text x="238" y="104" fill="#64748b" font-size="8.5" text-anchor="middle" font-family="'Chakra Petch', sans-serif">18:00</text>
            <text x="305" y="104" fill="#64748b" font-size="8.5" text-anchor="middle" font-family="'Chakra Petch', sans-serif">24:00</text>
          </svg>
        </div>
      </div>

      <!-- Bottom Parameter Telemetry Breakdown -->
      <div class="mt-1 pt-1.5 border-t border-[#184682]/60 grid grid-cols-3 gap-1.5 text-center">
        <div class="py-1 px-1 rounded-md bg-[#0a285e]/85 border border-[#235eae]/60 hover:border-cyan-400/50 transition-colors">
          <div class="text-[9.5px] text-slate-300 truncate font-medium">{{ currentMetricConfig.stat1Label }}</div>
          <div class="mt-0.5 font-tech font-bold text-xs text-cyan-300 whitespace-nowrap">{{ currentMetricConfig.stat1Val }}</div>
        </div>
        <div class="py-1 px-1 rounded-md bg-[#0a285e]/85 border border-[#235eae]/60 hover:border-cyan-400/50 transition-colors">
          <div class="text-[9.5px] text-slate-300 truncate font-medium">{{ currentMetricConfig.stat2Label }}</div>
          <div class="mt-0.5 font-tech font-bold text-xs text-white whitespace-nowrap">{{ currentMetricConfig.stat2Val }}</div>
        </div>
        <div class="py-1 px-1 rounded-md bg-[#0a285e]/85 border border-[#235eae]/60 hover:border-emerald-400/50 transition-colors">
          <div class="text-[9.5px] text-slate-300 truncate font-medium">{{ currentMetricConfig.stat3Label }}</div>
          <div class="mt-0.5 flex items-center justify-center gap-1 text-emerald-300 font-bold text-xs whitespace-nowrap font-tech">
            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_5px_#10b981]"></span>
      ...
      <div class="grid grid-cols-2 gap-1.5 my-1 flex-1">
        ...
        >
          ...
          <div class="relative w-12 h-12 flex-shrink-0 flex items-center justify-center">
            <svg class="w-full h-full -rotate-90 overflow-visible" viewBox="0 0 48 48">
              <!-- Base Track -->
              <circle cx="24" cy="24" r="18" fill="none" stroke="#0d3063" stroke-width="4.5" />
              <!-- Outer Dashed Tick Ring -->
              <circle cx="24" cy="24" r="22.5" fill="none" stroke="#1d4d8c" stroke-width="0.8" stroke-dasharray="2 3" opacity="0.55" />
              <!-- Progress Ring -->
              <circle
                cx="24"
                cy="24"
                ...
                :style="{ filter: `drop-shadow(0 0 6px ${dev.color}b3)` }"
              />
            </svg>
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center font-tech leading-none pointer-events-none">
              ...
              <span class="text-[8px] text-cyan-300 font-sans mt-0.5 font-medium">在网</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-1.5 my-1 flex-1">
        ...
        <div class="bg-[#0a285e]/85 hover:bg-[#123877] border border-[#235eae]/60 rounded-lg p-1.5 px-2 flex items-center justify-between transition-colors group">
          ...
          <div class="relative w-11 h-11 flex-shrink-0 flex items-center justify-center">
            <svg class="w-full h-full -rotate-90 overflow-visible" viewBox="0 0 44 44">
              <circle cx="22" cy="22" r="16" fill="none" stroke="#0d3063" stroke-width="4.2" />
              <circle cx="22" cy="22" r="20" fill="none" stroke="#1d4d8c" stroke-width="0.8" stroke-dasharray="2 2" opacity="0.45" />
              <circle
                cx="22"
                cy="22"
                r="16"
                ...
                stroke-linecap="round"
                class="drop-shadow-[0_0_6px_rgba(0,240,255,0.9)]"
              />
            </svg>
            <div class="absolute inset-0 flex items-center justify-center font-tech text-[10px] text-cyan-300 font-bold">
    ...
    <div class="tech-panel rounded-xl py-2 px-2.5 flex flex-col justify-between shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
      ...
      <div class="flex items-center gap-2 my-1 flex-1">
        ...
        <div class="flex flex-col items-center justify-center w-22 flex-shrink-0">
          <div class="relative w-[76px] h-[76px] flex items-center justify-center">
            <svg class="w-full h-full -rotate-90 overflow-visible" viewBox="0 0 88 88">
              <!-- Background Track -->
              <circle cx="44" cy="44" r="35" fill="none" stroke="#0d3063" stroke-width="7" />
              <!-- Outer Accent Tick Ring -->
              <circle cx="44" cy="44" r="41.5" fill="none" stroke="#1d4d8c" stroke-width="0.9" stroke-dasharray="3 3" opacity="0.65" />
              ...
              <circle
                ...
                stroke-linecap="round"
                class="drop-shadow-[0_0_10px_rgba(0,229,163,0.95)]"
              />
            </svg>
...
const currentMetricConfig = computed(() => {
  if (activeMetric.value === 'esd') {
    return {
      ...
      yMid: '1.0',
      yMin: '0.0',
      strokePath: 'M 35 76 C 50 76, 60 72, 70 72 C 85 72, 95 74, 105 74 C 120 74, 130 66, 140 66 C 155 66, 165 70, 175 70 C 190 70, 200 42, 210 42 C 220 42, 235 54, 245 54 C 260 54, 270 60, 275 60 C 285 60, 295 58, 305 58',
      areaPath: 'M 35 76 C 50 76, 60 72, 70 72 C 85 72, 95 74, 105 74 C 120 74, 130 66, 140 66 C 155 66, 165 70, 175 70 C 190 70, 200 42, 210 42 C 220 42, 235 54, 245 54 C 260 54, 270 60, 275 60 C 285 60, 295 58, 305 58 L 305 90 L 35 90 Z',
      peakPoint: { x: 210, y: 42, label: '1.20kV' },
      lastPoint: { x: 305, y: 58 },
      stat1Label: '静电峰值',
      stat1Val: '1.20 kV',
      stat2Label: '消散耗时',
      stat2Val: '0.8 s',
  ...
  if (activeMetric.value === 'ground') {
    return {
      ...
      yMid: '0.6',
      yMin: '0.2',
      strokePath: 'M 35 58 C 50 58, 60 60, 70 60 C 85 60, 95 56, 105 56 C 120 56, 130 59, 140 59 C 155 59, 165 55, 175 55 C 190 55, 200 57, 210 57 C 220 57, 235 59, 245 59 C 260 59, 270 57, 275 57 C 285 57, 295 58, 305 58',
      areaPath: 'M 35 58 C 50 58, 60 60, 70 60 C 85 60, 95 56, 105 56 C 120 56, 130 59, 140 59 C 155 59, 165 55, 175 55 C 190 55, 200 57, 210 57 C 220 57, 235 59, 245 59 C 260 59, 270 57, 275 57 C 285 57, 295 58, 305 58 L 305 90 L 35 90 Z',
      peakPoint: { x: 175, y: 55, label: '0.55Ω' },
      lastPoint: { x: 305, y: 58 },
      stat1Label: '工频电阻',
      stat1Val: '0.52 Ω',
      stat2Label: '冲击阻抗',
  ...
  return {
    ...
    yMid: '40',
    yMin: '0',
    strokePath: 'M 35 84 C 50 84, 55 78, 68 78 C 80 78, 90 81, 102 81 C 115 81, 125 70, 136 70 C 150 70, 160 58, 170 58 C 180 58, 190 68, 198 68 C 208 68, 215 31, 222 31 C 229 31, 238 50, 245 50 C 255 50, 265 72, 275 72 C 285 72, 295 66, 305 66',
    areaPath: 'M 35 84 C 50 84, 55 78, 68 78 C 80 78, 90 81, 102 81 C 115 81, 125 70, 136 70 C 150 70, 160 58, 170 58 C 180 58, 190 68, 198 68 C 208 68, 215 31, 222 31 C 229 31, 238 50, 245 50 C 255 50, 265 72, 275 72 C 285 72, 295 66, 305 66 L 305 90 L 35 90 Z',
    peakPoint: { x: 222, y: 31, label: '58.7kA' },
    lastPoint: { x: 305, y: 66 },
    stat1Label: '24h峰值',
    stat1Val: '58.7 kA',
    stat2Label: '放电积分',
    stat2Val: '14.8 C',
